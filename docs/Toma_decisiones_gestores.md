...
2.  **Consideraciones iniciales**: En un ecosistema tradicional (o híbrido) con TypeScript, conviene comparar las opciones siguiendo los requisitos clave anteriores. A continuación se resumen las características prácticas de cada alternativa:
    ...
    *   **`Bun`**: Enfoque todo-en-uno (runtime, gestor de paquetes, bundler y test runner) con rendimiento muy alto en instalaciones, arranque y resolución de dependencias. Guarda lockfile (`bun.lockb`). Es más joven que las alternativas, por lo que conviene fijar versiones en CI, añadir pruebas de compatibilidad con paquetes npm y preparar un plan de contingencia si surge alguna incompatibilidad.
...
1.  **Consideración inicial**:
    ...
    *   **`Bun`**: Bun usa directamente el archivo package.json, por lo tanto no se pueden configurar task que dependan de otras (obviando las tasks pre- y post-), paralelismo,... Aunque se han planteado mejoras en este respecto, no existe una implementación actual. Se facilita la integración con github actions.
